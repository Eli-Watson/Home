...
function v() {
  document.head.querySelectorAll("[data-reload]").forEach(y), document.body.querySelectorAll("script").forEach(y);
}
function y(o) {
  ...
  async reconstructDOM({ type: e, next: t, prev: r, scrollId: n }) {
    ...
    try {
      if (this.log("\u26A1", e), ["popstate", "link", "go"].includes(e) && t !== r) {
        ...
        q(c), this.opts.pageTransitions && document.createDocumentTransition ? document.createDocumentTransition().start(() => {
          b(c), v(), g(e, n);
        }) : (b(c), v(), g(e, n)), window.dispatchEvent(new CustomEvent("flamethrower:router:end")), setTimeout(() => {
          this.prefetch();
